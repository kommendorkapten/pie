 ...
 * @return 0 on success.
 */
static int write_dwn_smpl(struct pie_bm_f32rgb*, int, const char*, int);
                      ...
                      struct pie_mq_new_media* new)
{
        ...
        timing_start(&t);
        ok = pie_bm_load(&bm_src, src_path, io_opts);
                          ...
                          struct pie_mq_upd_proxy* upd)
{
        ...
        snprintf(path, PIE_PATH_LEN,
                 ...
                 min->min_path);
        timing_start(&t);
        ok = pie_bm_load(&bm, path, io_opts);
        if (ok)
        {
        ...
        if (ok == 0)
        {
                ...
                pie_alg_set_to_int_fmt(&dp);
                pie_alg_render(&bm, NULL, &dp);
        }
...
static int write_dwn_smpl(struct pie_bm_f32rgb* src,
                          int max_size,
                          const char* p,
                          int qual)
{
        struct pie_bm_f32rgb bm_dwn;
        struct pie_bm_u8rgb bm_out;
        struct pie_bm_f32rgb* bm_src = src;
        struct timing t;
        int ok;
        ...
        timing_start(&t);
        ok = pie_bm_jpg_u8rgb_write(p, &bm_out, qual);
