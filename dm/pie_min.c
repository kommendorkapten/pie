...
int
pie_min_read_hash(sqlite3 * db, struct pie_min * this)
{
	char           *q = "SELECT min_mob_id,min_added_ts_millis,min_stg_id,min_size,min_path FROM pie_min WHERE min_sha1_hash = ?";
	sqlite3_stmt   *pstmt;
	int             ret;
	int             retf;
	const unsigned char *c;
	...
	ret = sqlite3_bind_text(pstmt, 1, this->min_sha1_hash, -1, SQLITE_STATIC);
	if (ret != SQLITE_OK)
	{
		ret = -1;
		goto cleanup;
